...
export function PrivateLeaguePanel({
  authToken,
  ...
  const fetchPrivateLeagues = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/private-leagues?leagueSeasonId=${selectedLeagueSeason}`,
        {
  ...
  const handleLeaveLeague = async (leagueId) => {
    ...
    try {
      const response = await fetch(
        `/api/v1/private-leagues/${leagueId}/leave`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${authToken}` },
        },
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[#5f6a76]">
            Create your own league, share invite code, and track standings with
            friends.
          </p>
          <p className="mt-1 text-xs text-[#6a7683]">
            Limits: join up to 25 private leagues, create up to 5 private
            leagues.
          </p>
        </div>
        <button
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <form
          ...
          className="rounded-2xl border border-[#e4ddd2] bg-[#fffaf2] p-4"
        >
          <p className="text-sm font-semibold text-[#38424d]">
        ...
        <form
          ...
          className="rounded-2xl border border-[#e4ddd2] bg-[#eef9f5] p-4"
        >
          <p className="text-sm font-semibold text-[#38424d]">
            Join by Invite Code
          </p>
      ...
      {notice ? (
        <p className="mt-3 rounded-xl bg-[#f4efe5] px-3 py-2 text-xs font-semibold text-[#5a4d3b]">
          {notice}
        </p>
      ) : null}
      {error ? (
        <p className="mt-3 rounded-xl bg-[#fdecea] px-3 py-2 text-xs font-semibold text-[#8a3d35]">
          {error}
        </p>
      ) : null}
        ...
        {loading ? (
          ...
          leagues.map((league) => {
            ...
            return (
              <article
                key={league.id}
                className="rounded-2xl border border-[#e4ddd2] bg-white p-4"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-xs">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-1 font-semibold ${isAdmin ? "bg-[#fff3cd] text-[#856404]" : "bg-[#f0f3f6] text-[#58606a]"}`}
                      >
                        {String(league.myRole || "member").toUpperCase()}
                      </span>
                      {league.isOverall ? (
                        <span className="rounded-full bg-[#f7d9d5] px-2.5 py-1 font-semibold text-[#8a3d35]">
                          OVERALL
                        </span>
                      ) : null}
                    </div>
                ...
                {details ? (
                  <div className="mt-3 rounded-xl border border-[#e4ddd2] bg-[#faf6ef] p-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#6a7683]">
                    ...
                    details.standings.length ? (
                        ...
                        <table className="min-w-full text-left text-xs">
                          ...
                          <tbody>
                            {details.standings.map((item) => (
                              ...
                              >
                                <td className="px-2 py-1.5 font-semibold text-[#38424d]">
                                  #{item.rank}
                                </td>
                                ...
                                <td className="px-2 py-1.5 text-[#6a7683]">
                      ...
                      <p className="mt-2 text-xs text-[#5f6a76]">
                        No finalized standings yet.
                      </p>
                    )}
                  </div>
                ) : null}
