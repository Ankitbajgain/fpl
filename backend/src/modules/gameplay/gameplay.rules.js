...
const validateSquad = ({ players, captainId, viceCaptainId, budgetCap = 100 }) => {
  ...
  for (const p of players) {
    ...
    counts[p.role] += 1;
    franchiseCount[p.franchise_id] = (franchiseCount[p.franchise_id] || 0) + 1;
  }

  ensure(totalCredits <= budgetCap, `Budget exceeded. Used ${totalCredits}, cap ${budgetCap}`);
  ...
  for (const [franchiseId, c] of Object.entries(franchiseCount)) {
    ensure(c <= SQUAD_LIMITS.MAX_PER_FRANCHISE, `Too many players from franchise ${franchiseId}`);
  }

  const ids = new Set(players.map((p) => Number(p.id)));
  ensure(ids.has(Number(captainId)), 'Captain must be part of squad');
  ensure(ids.has(Number(viceCaptainId)), 'Vice-Captain must be part of squad');
