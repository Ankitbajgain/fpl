...
export default function App() {
  const [token, setToken] = useState(
    localStorage.getItem("dashboard_admin_token") || "",
  );
  ...
  const loadLeagues = async () => {
    if (!token) return;
    try {
      const data = await fetchJson("/api/v1/gameplay/leagues", {
  ...
  const loadLeagueData = async (leagueSeasonId) => {
    ...
    try {
      const [fixtureData, franchiseData, policyData] = await Promise.all([
        fetchJson(`/api/v1/admin/leagues/${leagueSeasonId}/fixtures`, {
          headers: authHeaders(token),
        }),
        fetchJson(`/api/v1/admin/leagues/${leagueSeasonId}/franchises`, {
          headers: authHeaders(token),
        }),
        fetchJson(`/api/v1/admin/leagues/${leagueSeasonId}/transfers/policy`, {
          headers: authHeaders(token),
        }),
      ]);
  ...
  const loadLeaderboard = async (leagueSeasonId) => {
    ...
    try {
      const data = await fetchJson(
        `/api/v1/gameplay/leagues/${leagueSeasonId}/leaderboard?playersLimit=20&managersLimit=20`,
        {
          headers: authHeaders(token),
        },
      );
  ...
  const handleUpdateFixture = async (fixtureId, form) => {
    ...
    try {
      await fetchJson(
        `/api/v1/admin/leagues/${selectedLeague}/fixtures/${fixtureId}`,
        {
          method: "PUT",
          headers: authHeaders(token),
          body: JSON.stringify(form),
  ...
  const handlePolicySave = async (form) => {
    ...
    try {
      const data = await fetchJson(
        `/api/v1/admin/leagues/${selectedLeague}/transfers/policy`,
        {
          method: "PUT",
          headers: authHeaders(token),
          body: JSON.stringify(form),
  ...
  const handlePushStats = async (fixtureId, statsArray) => {
    ...
    try {
      const data = await fetchJson(
        `/api/v1/admin/leagues/${selectedLeague}/fixtures/${fixtureId}/stats`,
        {
          method: "POST",
          headers: authHeaders(token),
  ...
  const handleFinalizePoints = async (fixtureId) => {
    ...
    try {
      const data = await fetchJson(
        `/api/v1/admin/leagues/${selectedLeague}/fixtures/${fixtureId}/finalize-points`,
        {
          method: "POST",
          headers: authHeaders(token),
        },
      ...
      notify(
        `Points finalized: ${data.squadsProcessed} squad(s), ${data.playersProcessed} player(s)`,
      );
      await loadLeagueData(selectedLeague);
  ...
  const handleSync = async (payload) => {
    ...
    try {
      const data = await fetchJson(
        `/api/v1/admin/leagues/${selectedLeague}/fixtures/sync`,
        {
          method: "POST",
          headers: authHeaders(token),
      ...
      await loadLeagueData(selectedLeague);
      notify(
        `Sync complete: inserted ${data.inserted}, updated ${data.updated}`,
      );
    } catch (e) {
      fail(e.message);
    } finally {
      ...
      <main className="shell auth-shell">
        <section className="auth-wrap">
          <div className="hero-card">
            <span className="pill">New FPL Admin</span>
            <h1>Control Room for Fixtures and League Operations</h1>
            <p>
              The design matches your main app so admin work feels familiar,
              guided, and easy to operate.
            </p>
            <ul className="hero-points">
              <li>Set up and update fixtures with clear labeled fields</li>
              <li>Manage transfer windows and policy settings</li>
              <li>Sync data and finalize points in just a few clicks</li>
            </ul>
          </div>
          <LoginPanel
            onLogin={login}
    ...
    <main className="shell">
      ...
      <section className="overview-grid">
        <div className="overview-card">
          <p className="k">Leagues</p>
          <p className="v">{leagues.length}</p>
        </div>
        <div className="overview-card">
          <p className="k">Fixtures</p>
          <p className="v">{dashboardOverview.totalFixtures}</p>
        </div>
        <div className="overview-card">
          <p className="k">Completed</p>
          <p className="v">{dashboardOverview.completedFixtures}</p>
        </div>
        <div className="overview-card">
          <p className="k">Live</p>
          <p className="v">{dashboardOverview.liveFixtures}</p>
        </div>
      </section>
        ...
        <div className="left-col">
          <div className="panel">
            ...
            <ul className="helper-list">
              <li>Select your working league first</li>
              <li>Add or edit fixtures with labeled fields</li>
              <li>Use Sync only when your source data is ready</li>
              <li>Review leaderboard updates after finalizing points</li>
            </ul>
          </div>
          <LeaguePicker
            leagues={leagues}
          ...
          />
          <ManagerLeaderboardPanel
            rows={managerLeaderboardRows}
            loading={managerLeaderboardLoading}
            error={managerLeaderboardError}
          />
